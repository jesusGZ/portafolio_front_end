...
export async function userInfo(id) {
	try {
		...
		const response = await fetch(`${CONEXION_BASE}${PORT}/user`, { method: 'GET', headers: headers });
...
export async function userLogin(user, password) {
	try {
		...
		const response = await fetch(`${CONEXION_BASE}${PORT}/user/login`, { method: 'GET', headers: headers, body: body });
